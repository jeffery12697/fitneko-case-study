...
The first is the user's right to their own data. When someone says erase me, it has to be erasure: no `is_deleted` flag, no archive table, no "we anonymized the display name." Anything short of that is a lie the schema tells about you.
...
Worse than being in tension, the two obligations are ordered badly:
...
| `invites` | both sides `→ NULL`, row kept | More than a relationship; see below. |
...
Its semantics are coarse on purpose: existence means this identity already collected its one-time rewards, so it never collects them again. Three payout paths consult it: the starter grant, achievement rewards, and accepting an invite code. A returning user gets a fresh account, logs normally, unlocks achievements, watches the sticker wall light up, and earns zero credits from any of it.
...
- **Every suppression check fails open.** A nil checker or a failed query means *pay the reward*. The system's default posture is to over-pay five credits rather than to greet a new user by withholding what they're owed, the same philosophy as every other entitlement gate here.
...
> Beyond *what does this row describe*, ask what is derived from this row's existence. A row that something else counts is a record in its own right, and deleting it silently edits that other thing's history.
...
A trigger written several phases earlier rejected both `UPDATE` and `DELETE` on the credit-transaction ledger. Which meant account deletion was impossible, in production and not only in a test, and would have rolled back the entire transaction.
...
## Making the flow honest as well as correct
...
- Confirmation is a typed word, not a second button. Deletion is instant and has no cooling-off period, which is a simplification I chose (no scheduled jobs, no half-deleted state) and therefore demands a gesture nobody performs by accident.
- Afterwards the app shows a full-page farewell with the bottom navigation removed, not merely disabled. Any tap into a tab would immediately create a fresh empty account for the person who just deleted theirs, which is a confusing way to undo someone's decision on their behalf.

## Trade-offs I accepted

- **The suppression flag is stamped at account creation.** It's a snapshot, so a tombstone written after an account exists doesn't retroactively suppress it. That can't happen through the product's own flow (the tombstone is written in the same transaction that removes the account), but it makes the flag a cached answer rather than a live one, and it's on the follow-up list to move the lookup into the three reward paths.
