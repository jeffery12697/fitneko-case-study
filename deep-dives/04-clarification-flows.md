...
- **Failures degrade, never propagate.** If the DynamoDB store errors, it's logged and treated as a miss. A flaky dependency can make the bot slightly forgetful, but it can never make the bot wrong.
...
The injected `clock` is a small thing that pays for itself: TTL expiry is unit-tested by advancing a fake clock instead of sleeping.
...
Case 3 is the one most implementations get wrong: they trap the user in a question they've moved past.
...
- **Deterministic follow-up matching only.** `ParseConsumedAmount` is regex based rather than LLM based, consistent with the project's wider rule: side-effectful decisions (what gets logged) prefer deterministic code, with the LLM upstream of them.
